...
def calculate_reward(L: float, D: float, parallelism_scheme: List[int], 
                    ...
                    target_data_constraint: float = 50.0,
...
class WordCountRealEnvironment:
    ...
    def __init__(self, 
        ...
        # 注意：不带组件标识，例如spout，bolt
        self.operator_mapping = {
            "dataSource": 0,  #  operator
            "dataCleasing": 1,  #  operator
            "GPSPare": 2 ,   #  operator
            "Prediction": 3
        }
        
        # Dependencies: (source_operator_id, target_operator_id)
        self.dependencies = [(0, 1), (1, 2),(1, 2)]  # dataSource -> dataCleasing -> GPSPare -> Prediction

        # Global operator instance count configuration
        self.operator_instance_counts = {
            "dataSource": 2,  #  operator
            "dataCleasing": 4,  #  operator
            "GPSPare": 4 ,   #  operator
            "Prediction": 4
        }
        ...
        self.topology_name = "Mydemo2"  # Default topology name
        ...
        logger.info("TaxiPreditionToplogy real environment initialized")
    ...
    def step(self, parallelism_scheme: List[int]) -> Tuple[float, Dict[str, Any]]:
        ...
        try:
            ...
            if rebalance_success:
                ...
                operator_names = ["dataSource", "dataCleasing", "GPSPare", "Prediction"]
    ...
    def _apply_rebalance(self, parallelism_scheme: List[int]) -> bool:
        ...
        try:
            ...
            component_parallelism = {
                "dataSource": parallelism_scheme[0],  # Source
                "dataCleasing": parallelism_scheme[1],   # Splitter
                "GPSPare": parallelism_scheme[2],     # Counter
                "Prediction": parallelism_scheme[3]     # Counter
            }
    ...
    def _ssh_rebalance(self, parallelism_scheme: List[int]) -> bool:
        ...
        try:
            ...
            remote_storm_cmd = f"{self.storm_bin_path} rebalance {self.topology_name} " \
                             f"-e dataSource-spout={parallelism_scheme[0]} -e dataCleasing-bolt={parallelism_scheme[1]} -e GPSPare-bolt={parallelism_scheme[2]} -e Prediction-bolt={parallelism_scheme[3]}" 
...
def LaStream_Agent_WC(deepseek_api_key: str, num_episodes: int = 10):
    ...
    try:
        ...
        for episode in range(num_episodes):
            ...
            # 检查选择的并行度方案是否与当前配置相同
            current_config = [
                env.operator_instance_counts["dataSource"],
                env.operator_instance_counts["dataCleasing"],
                env.operator_instance_counts["GPSPare"],
                env.operator_instance_counts["Prediction"]
            ]
            ...
            if modelcount > 0 and modelcount % 6 == 0:
                ...
                # Save model
                model_path = "taxipredition_real_rl_model.pt"
